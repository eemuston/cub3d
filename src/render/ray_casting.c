...
/*   Updated: 2023/10/09 16:35:36 by vvu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	shortest_distance(t_cub3d *data, double ray_angle)
{
	...
	if (data->ray->side == 0)
		data->ray->wall_distance = (data->ray->map_x - data->ray->ray_pos.p_x + (1 - data->ray->step_x) / 2) / data->ray->ray_dir.p_x;
	else
		data->ray->wall_distance = (data->ray->map_y - data->ray->ray_pos.p_y + (1 - data->ray->step_y) / 2) / data->ray->ray_dir.p_y;
