...
/*   Updated: 2023/10/14 14:58:56 by atoof            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_2d_direction(t_cub3d *data)
{
	...
	p2.p_x = (MINI_WIDTH / 2) + PLAYER_SIZE / 2 + data->player->pdx * 10;
	p2.p_y = (MINI_HEIGHT / 2) + PLAYER_SIZE / 2 + data->player->pdy * 10;
	dda_algorithm(p1, p2, data, line);
}

void	my_mlx_pixel_put_mini(t_cub3d *data, double width, double height,
		unsigned int color)
{
	char	*dst;
	int		y;
	...
	dst = data->mini_img->addr + (y * data->mini_img->line_length + x
			* (data->mini_img->bits_per_pixel / 8));
